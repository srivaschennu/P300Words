function stat = clustcorr(stat,varargin)
...
for n = 1:size(stat.gfpdiff,1)
    ...
    for p = corrwin
        ...
        pprob(p) = sum(stat.gfpdiff(:,p) >= stat.gfpdiff(n,p))/size(stat.gfpdiff,1);
        nprob(p) = sum(stat.gfpdiff(:,p) <= stat.gfpdiff(n,p))/size(stat.gfpdiff,1);
    end
