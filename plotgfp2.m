...
for s = 1:length(stats)
    ...
    tct = load(sprintf('%s/%s_%s_%s_%d-%d_tct.mat',filepath,stats{s}.statmode,num2str(stats{s}.subjinfo),stats{s}.condlist{1},stats{s}.param.latency));
    if ~isempty(stats{s}.pclust)
        text(0,-0.9,sprintf('%dms, tct p = %.3f\nt = %.1f, p = %.3f',round(stats{s}.times(stats{s}.plotpnt)),tct.stat.pclust.prob,stats{s}.pclust(1).tstat, stats{s}.pclust(1).prob),...
            'FontSize',param.fontsize,'FontName',fontname,'HorizontalAlignment','center');
    else
        text(0,-0.9,sprintf('%dms, tct p = %.3f',round(stats{s}.times(stats{s}.plotpnt)),tct.stat.pclust.prob),...
            'FontSize',param.fontsize,'FontName',fontname,'HorizontalAlignment','center');
        %         text(0,-0.9,sprintf('%dms\nt = %.1f, p = %.3f',round(stats{s}.times(stats{s}.plotpnt)),stats{s}.valu(stats{s}.plotpnt),stats{s}.pprob(stats{s}.plotpnt)),...
        %             'FontSize',param.fontsize,'FontName',fontname,'HorizontalAlignment','center');
    end
end
